...
module.exports = function(nfa) {
  ...
  const dfaAcceptState = new Array();
  const dfaAcceptStateFinal = new Array();
  const nfaAlphabet = [];
  ...
  function getEClosure(nfatran, eclosure, state_num) {
    for (let k = 0; k < state_num; k++) {
      ...
      while (Etemp[j] != undefined) // 找新闭包
      {
        for (i = 0; nfatran[i] != undefined; i++)// 查看每条边，先找iεX
        {
          if (nfatran[i].startState == Etemp[j] && nfatran[i].inputChar == 'ε') {
            // 判断是否将新状态放入Etemp
            for (let n = 0; Etemp[n] != undefined; n++) {
      ...
      for (i = 0; Etemp[i] != undefined; i++) {
        let min = i;
        for (let j = i; Etemp[j] != undefined; j++) {
  ...
  let dfa_mid = new Array();

  function getDFA(eclosure, dfa_mid, nfa_tran, alphabet) {
    const Eadd = new Array();
    let EaddN = 0;
    let dfa_tran_num = 0;
    dfa_mid[dfa_tran_num] = new dfa_mid_StateTransition(eclosure[0], undefined, undefined);// dfa初始状态为0的闭包
    const dfa_start_state = new Array();
    dfa_start_state.push(eclosure[0]);
    // console.log("dfa_start_state[0]");
    // console.log(dfa_mid);

    for (let num = 0; dfa_start_state[num] != undefined; num++)// 开始计算move（A，a），move（A,b）......dfa_start_state[num]!=undefined
    {
      // console.log("dfa_start_state[num]",dfa_start_state[num]);
      for (let c = 0; alphabet[c] != 'ε' && alphabet[c] != undefined; c++)// 每个转换条件都计算一遍
      {
        const newEState = new Array();
        for (let i = 0; dfa_start_state[num][i] != undefined; i++)// 从dfa一个新集合开始（0的闭包开始）
        {
          for (let j = 0; nfa_tran[j] != undefined; j++)// 循环每条nfa转换
          {
            if (nfa_tran[j].startState == dfa_start_state[num][i] && nfa_tran[j].inputChar == alphabet[c]) {
              // //
              if (newEState.length == 0) {
                newEState.push(nfa_tran[j].endState);
              }
              for (let k = 0; k < newEState.length; k++) {
        ...
        // 闭包相加
        const tempEadd = new Array();
        for (i = 0; newEState[i] != undefined; i++) {
          const state = newEState[i];
          for (j = 0; eclosure[state][j] != undefined; j++) {
            tempEadd.push(eclosure[state][j]);
          }
        }
        // 闭包相加后对闭包进行排序
        for (let k = 0; k < tempEadd.length; k++) {
          let min = k;
          for (let q = k + 1; q < tempEadd.length; q++) {
            if (tempEadd[min] > tempEadd[q]) {
              min = q;
            }
          }
          const temp = tempEadd[k];
          ...
          for (i = 0; dfa_start_state[i] != undefined; i++) {
            for (j = 0; Eadd[EaddN - 1][j] != undefined; j++) {
              if (Eadd[EaddN - 1][j] != dfa_start_state[i][j]) {
                break;
              } else if (Eadd[EaddN - 1][j + 1] == undefined && dfa_start_state[i][j + 1] == undefined) {
                isadd = 0; break;
              }
            }
          }
  ...
  function getDFA2(dfa_mid, dfa_tran) {
    ...
    for (let i = 0; i < dfaAcceptState.length; i++)// 遍历要去多余的表
    {
      if (i == 0) {
        dfaAcceptStateFinal.push(dfaAcceptState[0]);
      }// 默认先弹入dfaAcceptState的第一条到dfaAcceptStateFinal
      else {// 判断遍历到的dfaAcceptState是否有相同的state在dfaAcceptTableFinal里,并记录是哪条dfaAcceptStateFinal
        let isin = 0;
        const updateState = 0;
        for (let j = 0; j < dfaAcceptStateFinal.length; j++) {
          if (dfaAcceptStateFinal[j].state == dfaAcceptState[i].state) {
            isin = 1; whichState = j; break;
          }
        }
        if (isin == 1) {// 如果有则判断REid大小，将较小的REid更新到dfaAcceptTableFinal里
          if (dfaAcceptStateFinal[updateState].REId > dfaAcceptState[i].REId) {
            dfaAcceptStateFinal[updateState].REId = dfaAcceptState[i].REId;
          }
        } else// 如果没有则push
        {
