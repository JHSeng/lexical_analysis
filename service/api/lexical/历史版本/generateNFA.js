...
addStateTransition = function (start, input, end) {
  const temp = new StateTransition();
...
printNFA = function () {
  ...
  for (let i = 0; i < this.stateSteam.length; i++) {
...
const regularExpression = function (str) {
...
function connectOperator() {
  ...
  const op3 = new NFA(op2.startState, op1.endState);
...
function updateStateTable1() {
  ...
  let new_state = 0;
  const start = this.NFAStark.peek().startState;
  ...
  for (var i = 0; i < this.NFAStark.peek().stateSteam.length; i++) {
    if (this.NFAStark.peek().stateSteam[i].startState == start) {
      ...
      const end = this.NFAStark.peek().stateSteam[i].endState;
      let temp = 0;
      for (var j = 0; j < this.stateTable1.length; j++) {
...
function updateStateTable2(start, new_state, self) {
  // console.log(self)
  for (let i = 0; i < self.NFAStark.peek().stateSteam.length; i++) {
    if (self.NFAStark.peek().stateSteam[i].startState == start) {
      // 表示state++  存在 并且可以使用
      let temp = 0;
      const end = self.NFAStark.peek().stateSteam[i].endState;
      for (let j = 0; j < self.stateTable1.length; j++) {
...
const output = function (strArray) {
  this.startState = 0;
  this.state = 1;
  this.stateSteam = [];
...
function generateTransitionTable() {
  ...
  for (var i = 0; i < this.stateSteam.length; i++) {
    const s = this.stateSteam[i].startState;
    const input = this.stateSteam[i].inputChar;
    var input1;
    for (var j = 0; j < this.alphabet.length; j++) {
      if (this.alphabet[j] == input) {
        input1 = j; break;
      }
    }
    const e = this.stateSteam[i].endState;
