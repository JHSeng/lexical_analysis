...
module.exports = function(NFAdata) {
  ...
  for (let i = 0; i < NFAdata.alphabet.length; i++) {
  ...
  if (existOfNull != -1) {
    ...
    code_str += 'let kTransitionTable = [\n';
    for (let i = 0; i < NFAdata.transitionTable.length; i++) {
      let temp_str = '  [ ';

      if (i + 1 == NFAdata.transitionTable.length) {
        for (let j = 0; j < NFAdata.transitionTable[i].length; j++) {
          let temp_str1 = '[';
          if (j + 1 == NFAdata.transitionTable[i].length) {
            for (let k = 0; k < NFAdata.transitionTable[i][j].length; k++) {
              if (k == 0) {
    ...
    code_str += '];\n\n';
    code_str += 'let kAcceptTable = [\n';
    for (let i = 0; i < NFAdata.transitionTable.length; i++) {
      let endOrNot = false;
      for (let j = 0; j < NFAdata.acceptStateList.length; j++) {
    ...
    code_str +=
      ...
      '      return false;\n' +
      '    }else{\n' +
      '      let closure=[]  //用于临时存储状态转移后 状态的ε闭包\n' +
      '      for(let k=0;k<closureOfState.length;k++){\n' +
      '        // 遍历状态表kTransitionTable获取当前闭包 在输入字符后的所有状态 并将状态添加入closure中\n' +
      '        for(let l=0;l<kTransitionTable[closureOfState[k]][j].length;l++){\n' +
      '          let existOrNot=false;\n' +
      ...
      '      }\n' +
      '      //获取closure状态集合的ε闭包\n' +
      '      for(let n=0 ;n<closure.length;n++){\n' +
      '        for(let o=0;o<kTransitionTable[closure[n]][indexOfNull].length;o++) {\n' +
      '          let existOrNot=false;\n' +
      '          for (let p=0;p<closure.length;p++){\n' +
    ...
    for (let i = 0; i < NFAdata.transitionTable.length; i++) {
      if (i + 1 == NFAdata.transitionTable.length) {
        let str = ' {';
        for (let j = 0; j < NFAdata.alphabet.length; j++) {
          if (j == 0) {
