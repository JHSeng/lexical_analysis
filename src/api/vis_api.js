...
function createEdges (transitionTable, alphabet, isHidden, openPlug = true) {
  var Edges = [];
    ...
    for (var chIndex of _range(alphabet.length)) {
      for (var tState of transitionTable[fState][chIndex]) {
        if (transitable_hack[fState][chIndex].indexOf(tState) != -1) {
          ...
          if (isHidden) {
            Edges.push({ id: Edges.length, from: fState, to: tState, arrows: "to", label: alphabet[chIndex], color: { color: "white" } });
          } else {
            Edges.push({ id: Edges.length, from: fState, to: tState, arrows: "to", label: alphabet[chIndex] });
          }
        } else {
          if (alphabet.indexOf("ε") != -1)
          // console.log(`edge: from ${fState} to ${tState}, not in transitable_hack`)
          {
            if (isHidden) {
              Edges.push({ id: Edges.length, from: fState, to: tState, arrows: "to", label: alphabet[chIndex], color: { color: "white" }, smooth: { type: "curvedCW", roundness: 0.5 } });
            } else {
...
function deepcopy (arr) {
   ...
   */
  var out = []; var i = 0; var len = arr.length;
  for (; i < len; i++) {
